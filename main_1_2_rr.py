...
def backtest_strategy(df, pivots, detailed_log_count=3, validation_file="detailed_validation.txt"):
    ...
    validation_file = "trade_validation_log_1_2_rr.txt"
    detailed_log_count = 5  # Number of trades to log in detail
    trades_logged = 0
    
    with open(validation_file, 'w') as val_file:
        val_file.write("=" * 80 + "\n")
        val_file.write("DETAILED TRADE VALIDATION LOG - FIRST 5 TRADES (1:2 RR EXIT VERSION)\n")
        val_file.write("=" * 80 + "\n")
        val_file.write("Purpose: Validate minute-by-minute strategy logic with 1:2 RR exit\n")
    ...
    logger.info(f"Starting 1:2 RR backtest with {len(pivot_highs)} pivot highs and {len(pivot_lows)} pivot lows")
    ...
    if detailed_log_count > 0:
        with open(validation_file, 'w') as val_file:
            val_file.write("5-MINUTE PIVOT BREAKOUT STRATEGY - DETAILED TRADE VALIDATION (1:2 RR EXIT)\n")
            val_file.write("="*80 + "\n")
            ...
            val_file.write("Risk Management: 1:2 RR target, then exit at target or SL\n")
            val_file.write("Exit Logic: Fixed 1:2 RR target or original SL\n")
            val_file.write("Pivot Usage: Reusable - allows multiple breakout attempts until pivot updates\n")
            val_file.write("Dataset: NIFTY 1-min data with 1:2 RR exit validation logic\n")
            val_file.write("="*80 + "\n\n")
    ...
    def write_trade_summary():
        ...
        if trades_logged < detailed_log_count and current_trade_bars:
            with open(validation_file, 'a') as val_file:
                trades_logged += 1
                val_file.write(f"\n" + "="*80 + "\n")
                val_file.write(f"TRADE SETUP #{trades_logged} VALIDATION (1:2 RR EXIT)\n")
                val_file.write("="*80 + "\n")
    ...
    for i in tqdm(range(len(df)), desc="Processing bars (1:2 RR)", unit="bars"):
        row = df.iloc[i]
        dt = row['datetime']
        ...
        remaining_trades = []
        for trade_info in open_trades:
            exit_result = check_trade_exit_1_2_rr(row, trade_info)
            if exit_result:
    ...
    logger.info(f"1:2 RR Backtest completed. Total trades: {len(trades)}")
...
def check_trade_exit_1_2_rr(row, trade_info):
    """Check if trade should exit using 1:2 RR target or stop loss"""
    if not trade_info['entered']:
        return None
...
def create_trade_result_from_exit(df, exit_idx, trade_info, exit_result, current_capital_ref):
    ...
    # Check if 1:2 target was achieved
    rr_target_achieved = exit_result['type'] == 'TP'
    ...
    return {
        ...
        'rr_target_achieved': rr_target_achieved,  # Track if 1:2 was reached
...
def calculate_yearly_analytics(trades, df):
    ...
    print(f"\n{'='*80}")
    print("YEAR-WISE DETAILED ANALYTICS (1:2 RR VERSION)")
    print(f"{'='*80}")
    ...
    for year in years:
        ...
        # 1:2 RR-specific stats
        tp_exits = len(year_trades[year_trades['result'] == 'TP'])
        ...
        print(f"  Max DD: {max_drawdown:>6.1f} pts | TP Exits: {tp_exits:>3} | SL Exits: {sl_exits:>3}")
...
def print_monthly_heatmap(trades):
    ...
    print(f"\n{'='*100}")
    print("MONTHLY P&L HEATMAP (Points) - 1:2 RR VERSION")
    print(f"{'='*100}")
    ...
    for year in years:
        ...
        for month_num in range(1, 13):
            ...
            if month_pl > 0:
                print(f"\033[92m{month_pl:>8.1f}\033[0m", end='')  # Green for positive
            elif month_pl < 0:
                print(f"\033[91m{month_pl:>8.1f}\033[0m", end='')  # Red for negative
            else:
                print(f"{month_pl:>8.1f}", end='')  # Default for zero
...
def print_trade_details(trades):
    print("\n" + "="*110)
    print("DETAILED TRADE ANALYSIS (1:2 RR VERSION)")
    print("="*110)
...
def print_analytics(analytics):
    print("\n" + "="*70)
    print("BACKTEST ANALYTICS SUMMARY (1:2 RR VERSION)")
    print("="*70)
    ...
    print("="*50)
    print(f"Target Price Exits: {analytics['tp_exits']}")
...
def main():
    ...
    print("="*70)
    print("5-MINUTE PIVOT BREAKOUT STRATEGY BACKTEST (1:2 RR VERSION)")
    print("="*70)
    ...
    print("- Exit Logic: 1:2 Risk-Reward target or stop loss")
    print("="*70)
    ...
    print(f"Loaded {len(df)} rows of data from {df['datetime'].min()} to {df['datetime'].max()}")
    
    # Calculate pivots
    pivots = calculate_pivots(df)
    print(f"Calculated {len(pivots)} pivot points")
    ...
    # Print trade details (first 50 trades)
    print_trade_details(trades[:50])
    
    # Calculate and print yearly analytics
    yearly_analytics = calculate_yearly_analytics(trades, df)
    ...
    trades_df = pd.DataFrame(trades)
    trades_df.to_csv('backtest_results_1_2_rr.csv', index=False)
    
    yearly_df = pd.DataFrame(yearly_analytics).T
    yearly_df.to_csv('yearly_analytics_1_2_rr.csv')
    
    print(f"\nYearly analytics exported to 'yearly_analytics_1_2_rr.csv'")
    print(f"Trade details exported to 'backtest_results_1_2_rr.csv'")
    print("Detailed logs available in the console output above")
