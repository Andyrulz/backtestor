...
def backtest_strategy(df, pivots, detailed_log_count=3, validation_file="detailed_validation.txt"):
    ...
    validation_file = "trade_validation_log.txt"
    detailed_log_count = 5  # Number of trades to log in detail
    trades_logged = 0
    
    with open(validation_file, 'w') as val_file:
        val_file.write("=" * 80 + "\n")
        val_file.write("DETAILED TRADE VALIDATION LOG - FIRST 5 TRADES\n")
        val_file.write("=" * 80 + "\n")
        val_file.write("Purpose: Validate minute-by-minute strategy logic\n")
    ...
    if detailed_log_count > 0:
        with open(validation_file, 'w') as val_file:
            val_file.write("5-MINUTE PIVOT BREAKOUT STRATEGY - DETAILED TRADE VALIDATION\n")
            val_file.write("="*80 + "\n")
            ...
            val_file.write("Risk Management: 2:1 RR, 0.1% of entry SL limit, any SL extension trigger\n")
            val_file.write("Pivot Usage: Reusable - allows multiple breakout attempts until pivot updates\n")
            val_file.write("Dataset: NIFTY 1-min data with backtest validation logic\n")
            val_file.write("="*80 + "\n\n")
    ...
    def write_trade_summary():
        ...
        if trades_logged < detailed_log_count and current_trade_bars:
            with open(validation_file, 'a') as val_file:
                trades_logged += 1
                val_file.write(f"\n" + "="*80 + "\n")
                val_file.write(f"TRADE SETUP #{trades_logged} VALIDATION\n")
                val_file.write("="*80 + "\n")
...
def check_trade_exit(row, trade_info, recent_bars=None):
