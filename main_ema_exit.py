...
def backtest_strategy(df, pivots, detailed_log_count=3, validation_file="detailed_validation.txt"):
    ...
    validation_file = "trade_validation_log_ema.txt"
    detailed_log_count = 5  # Number of trades to log in detail
    trades_logged = 0
    
    with open(validation_file, 'w') as val_file:
        val_file.write("=" * 80 + "\n")
        val_file.write("DETAILED TRADE VALIDATION LOG - FIRST 5 TRADES (EMA EXIT VERSION)\n")
        val_file.write("=" * 80 + "\n")
        val_file.write("Purpose: Validate minute-by-minute strategy logic with 10 EMA exit\n")
    ...
    logger.info(f"Starting EMA backtest with {len(pivot_highs)} pivot highs and {len(pivot_lows)} pivot lows")
    ...
    if detailed_log_count > 0:
        with open(validation_file, 'w') as val_file:
            val_file.write("5-MINUTE PIVOT BREAKOUT STRATEGY - DETAILED TRADE VALIDATION (EMA EXIT)\n")
            val_file.write("="*80 + "\n")
            ...
            val_file.write("Risk Management: Initial 1:2 RR, then 10 EMA exit, 0.1% of entry SL limit\n")
            val_file.write("Exit Logic: SL until 1:2 achieved, then 10 EMA close exit\n")
            val_file.write("Pivot Usage: Reusable - allows multiple breakout attempts until pivot updates\n")
            val_file.write("Dataset: NIFTY 1-min data with EMA exit validation logic\n")
            val_file.write("="*80 + "\n\n")
    ...
    def log_detailed_bar(i, row, dt, action=""):
        ...
        if len(current_trade_bars) > 0 or action.startswith("LONG BREAKOUT") or action.startswith("SHORT BREAKOUT"):
            ...
            if marking_candle_active:
                ...
                bar_info += " | Entry: - | SL: -"
            bar_info += f" | EMA10: {row['ema_10']:.1f}"
            if action:
                bar_info += f" | {action}"
    ...
    def write_trade_summary():
        ...
        if trades_logged < detailed_log_count and current_trade_bars:
            with open(validation_file, 'a') as val_file:
                trades_logged += 1
                val_file.write(f"\n" + "="*80 + "\n")
                val_file.write(f"TRADE SETUP #{trades_logged} VALIDATION (EMA EXIT)\n")
                val_file.write("="*80 + "\n")
                for bar_info in current_trade_bars:
                    val_file.write(bar_info + "\n")
                val_file.write("\n")
    
    for i in tqdm(range(len(df)), desc="Processing bars (EMA)", unit="bars"):
        row = df.iloc[i]
        dt = row['datetime']
        ...
        remaining_trades = []
        for trade_info in open_trades:
            exit_result = check_trade_exit_ema(row, trade_info)
            if exit_result:
            ...
            if marking_direction == 'long':
                if row['high'] > marking_entry:  # HIGH > ENTRY (not >=)
                    ...
                    active_trade_info = {
                        ...
                        'rr_achieved': False  # Track if 1:2 has been achieved
                    }
        ...
        # VALIDATION SCRIPT LOGIC - STEP 4: Update marking levels (only if no entry triggered this bar)
            ...
            # Only allow updates within 18 bars from breakout (removed 3-update limit)
    ...
    logger.info(f"EMA Backtest completed. Total trades: {len(trades)}")
    return trades
...
def check_trade_exit_ema(row, trade_info, recent_bars=None):
    """Check if trade should exit at SL or 10 EMA after 1:2 achieved (no breakeven SL movement)"""
    if not trade_info['entered']:
        return None
    ...
    if direction == 'long':
        # Check if 1:2 target achieved (but don't exit at 1:2)
        if not trade_info['rr_achieved'] and row['high'] >= tp_price:
            trade_info['rr_achieved'] = True
            # NOTE: DO NOT move SL to breakeven - keep original SL for better trend following
            
        # Always check original SL (before and after 1:2)
        if row['low'] <= sl_price:
            return {'type': 'SL', 'price': sl_price}
            
        # After 1:2 - also check EMA exit with buffer
        if trade_info['rr_achieved'] and row['close'] < (row['ema_10'] - 0.25):
            return {'type': 'EMA', 'price': row['close']}
            
    else:  # short
        # Check if 1:2 target achieved (but don't exit at 1:2)
        if not trade_info['rr_achieved'] and row['low'] <= tp_price:
            trade_info['rr_achieved'] = True
            # NOTE: DO NOT move SL to breakeven - keep original SL for better trend following
...
def create_trade_result_from_exit(df, exit_idx, trade_info, exit_result):
    ...
    exit_size = exit_result.get('size', 1.0)  # Default to full size if not specified
    
    # Calculate P&L based on exit size
    if trade_info['direction'] == 'long':
        pl_points = (exit_price - entry_price) * exit_size
    else:
        pl_points = (entry_price - exit_price) * exit_size
    ...
    return {
        ...
        'rr_achieved': pl_points / (sl_distance * exit_size) if sl_distance > 0 else 0,
        ...
        'exit_size': exit_size,  # Track what portion of position was closed
...
def close_trade_at_market_close(df, close_idx, trade_info):
    ...
    return create_trade_result_from_exit(df, close_idx, trade_info, exit_result)
...
def calculate_analytics(trades, df):
    ...
    # EMA-specific statistics
    ema_exits = sum(1 for t in trades if t['result'] == 'EMA')
...
def calculate_yearly_analytics(trades, df):
    ...
    print(f"\n{'='*80}")
    print("YEAR-WISE DETAILED ANALYTICS (EMA EXIT VERSION)")
    print(f"{'='*80}")
    ...
    for year in years:
        ...
        # EMA-specific stats
        ema_exits = len(year_trades[year_trades['result'] == 'EMA'])
        ...
        print(f"  Max DD: {max_drawdown:>6.1f} pts | EMA Exits: {ema_exits:>3} | SL Exits: {sl_exits:>3}")
...
def print_monthly_heatmap(trades):
    ...
    print(f"\n{'='*100}")
    print("MONTHLY P&L HEATMAP (Points) - EMA EXIT VERSION")
    print(f"{'='*100}")
...
def print_trade_details(trades):
    print("\n" + "="*110)
    print("DETAILED TRADE ANALYSIS (EMA EXIT VERSION)")
    print("="*110)
...
def print_analytics(analytics):
    print("\n" + "="*70)
    print("BACKTEST ANALYTICS SUMMARY (EMA EXIT VERSION)")
    print("="*70)
    ...
    print("="*50)
    print(f"EMA Exits: {analytics['ema_exits']}")
...
def main():
    ...
    print("="*70)
    print("5-MINUTE PIVOT BREAKOUT STRATEGY BACKTEST (EMA EXIT VERSION)")
    print("="*70)
    ...
    print("- EXIT LOGIC: SL until 1:2 achieved, then 10 EMA close exit")
    ...
    if trades:
        ...
        trades_df = pd.DataFrame(trades)
        trades_df.to_csv('backtest_results_ema.csv', index=False)
        
        # Export yearly statistics
        if yearly_stats:
            yearly_df = pd.DataFrame.from_dict(yearly_stats, orient='index')
            yearly_df.to_csv('yearly_analytics_ema.csv')
            print(f"\nYearly analytics exported to 'yearly_analytics_ema.csv'")
        
        print(f"Trade details exported to 'backtest_results_ema.csv'")
